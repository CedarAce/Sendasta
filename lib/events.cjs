...
//
// CommonJS (.cjs) on purpose: Vercel compiles the api/* handlers from ESM to
// CommonJS, and a compiled require() cannot load an ESM (.mjs) module.

const ADDIN_ACTIONS = new Set([
  "scan_started",
  "email_blocked",
  "email_allowed",
]);
...
function normalizeEvent(body, geo = {}) {
  ...
  if (geo.userAgent && row.user_agent == null) row.user_agent = geo.userAgent;

  return row;
}

module.exports = { ADDIN_ACTIONS, normalizeEvent };
